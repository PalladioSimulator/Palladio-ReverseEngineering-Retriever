...
public class EMFTextRuleHelper {
    ...
    public static boolean isObjectAnnotatedWithName(AnnotationInstanceOrModifier mod, String name) {
        if (mod.eClass()
            ...
            final String annoName = anno.getAnnotation()
                .getName();
    ...
    private static boolean isObjectModifiedExactlyWith(AnnotableAndModifiable modi, String... names) {
        ...
        for (final Modifier m : modi.getModifiers()) {
            final String modifierName = m.getClass()
                .getSimpleName()
                .toLowerCase();
            for (final String name : names) {
    ...
    public static List<Interface> getAllInterfaces(CompilationUnitImpl unit) {
        ...
        if (unit.getName()
            .contains("LocalDateAttr")) {
            final Class blubi = (Class) unit.getClassifiers()
                .get(0);
            StringBuilder debugSB = new StringBuilder();
            debugSB.append("[LocalDateAttr]:\n");
            blubi.getImplements()
                .forEach(im -> {
                    debugSB.append("\timp: ");
                    debugSB.append(im);
                    debugSB.append(", target: ");
                    debugSB.append(im.getPureClassifierReference()
                        .getTarget());
                    debugSB.append('\n');
                });
    ...
    public static Classifier getExtends(CompilationUnitImpl unit) {
        ...
        if (classi instanceof Class) {
            ...
            final TypeReference reference = cl.getExtends();
            return reference.getPureClassifierReference()
                .getTarget();
        }
        return null;
    }
